...
# Get the model predictions
            ...
            # Display the result
            for class_name, intervals in sorted_result:
                print(f"Class: {class_name}, Intervals: {intervals}")
                        # Return the formatted response
            response_str = "\n".join([f"Class: {class_name}, Intervals: {intervals}" for class_name, intervals in sorted_result])
...
class RealTimeDetectionManager:
    def __init__(self, node):#,input_device_number
        ...
        model = tf.saved_model.load("src/audionodepackage/resources/model/yamnet_ray") #path to model
    ...
    def _audio_callback(self, in_data, frame_count, time_info, status):
            ...
            if top_score >= score_threshold:
                self.node.get_logger().info(f"The main sound is: {top_class} with a score of: {top_score:.2f}")
